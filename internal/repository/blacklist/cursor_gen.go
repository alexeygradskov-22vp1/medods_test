...
func (c *Cursor) fetchRows() error {
	...
	c.rows = make([]Blacklist, 0, c.limit)
	for rows.Next() {
		var item Blacklist
		err := rows.Scan(
			&item.AccessToken,
...
func (c *Cursor) Current() (Blacklist, error) {
	if c.closed {
		return Blacklist{}, errors.New("cursor is closed")
	}

	if c.index >= 0 && c.index < len(c.rows) {
		return c.rows[c.index], nil
	}
	return Blacklist{}, errors.New("not found")
}
